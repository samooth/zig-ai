...
| Benchmarks paged-attention | ✅ (`benchmarks/bench_paged_attention.zig` vía `zig build bench-pa`: GPU-only by default (si CUDA disponible), fallback a CPU si no. Flags: `--cpu` (fuerza CPU+GPU), `--gpu-only` (fuerza GPU-only). Prefill latencia/throughput CPU+GPU vs batch, VRAM vs num_blocks, prefix hit rate, CPU offload. CPU prefill usa seq_len=64 (O(n²) reducido); GPU usa seq_len=512) | |
...
2. **Hecho**: Fase 2 — Kernels CUDA de PagedAttention (`paged_attention.cu` con `extern "C"`): decode (online softmax, warp-reduction, layout BlockAllocator), reshape_and_block_write, block_copy. Motor `PagedAttentionGpu` (patrón cuModuleLoad, como `GgufDequantEngine`) en `gpu_kernels.zig` + build_options con ruta al cubin instalado (`paged_attention.cubin`; la arquitectura GPU se auto-detecta en build vía `nvidia-smi`, configurable con `-Dgpu-arch`). Test `test_paged_attention_gpu.zig`: GPU decode/prefill vs CPU reference, max_diff ≈ 2.5e-4. (commit `9c98ffb`)
...
10. **Hecho**: Pool de bloques GPU paginado (paged-memory CUDA: `cuMemAddressReserve` / `cuMemCreate` / `cuMemMap` / `cuMemSetAccess`). `PagedGpuBlockPool` reserva VA para `num_blocks * block_bytes` y hace commit físico **por bloque** de forma perezosa: `stageBlock` crea un handle (`cuMemCreate`), lo mapea (`cuMemMap` + `cuMemSetAccess` con `CU_MEM_ACCESS_FLAGS_PROT_READWRITE`) y copia H2D; `evictBlock` copia D2H, desmapea y libera el handle. Solo los bloques residentes consumen memoria física del dispositivo. Requiere `block_bytes` alineado a la granularidad mínima (2MB en esta GPU, reportada por `cuMemGetAllocationGranularity`). `PagedAttentionGpu.ensurePool` intenta VMM primero y cae a `GpuBlockPool` contiguo si no está alineado. Bindings VMM añadidos a `cudaz_stub.zig` (tipos `CUmemAllocationProp`/`CUmemAccessDesc` + funciones reserve/create/map/setAccess/unmap/release/granularity). Test: `Paged GPU block pool (VMM) stage/evict round-trip preserves host data` (deriva `block_size` de la granularidad real).
11. **Hecho**: Suite completa verde con modelo real — `zig build test` con `GGUF_MODEL_PATH` ejecuta los 3 tests E1/E2 (carga real, tokenizer BPE, forward híbrido capa 3: `max_abs=0.661`) además de los 7 tests GPU de paged-attention. Tests GPU arreglados para Zig 0.16: `PagedAttentionGpu.init` ahora recibe un stream CUDA real (`cudaz.ensureContext()` + `cuStreamCreate(0)`, destruido tras `deinit()`) en lugar de `@ptrCast(@as(u64,0))`; los fixtures SSM de `ssm.zig` rellenan `scratch_qkv/z/out` con `dequantToF32Transposed` igual que `loadWeightsFromGguf`. (commit `be048c3`)
