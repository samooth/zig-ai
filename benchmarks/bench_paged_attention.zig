...
fn benchPrefillBatch(allocator: std.mem.Allocator, batch: usize, seq_len: usize, iters: usize, gpu_only: bool) !struct { cpu_ms: f64, cpu_toks: f64, gpu_ms: f64, gpu_toks: f64 } {
    ...
    debug.dbg.printLevel(.info, "[bench_pa] prefill: creating KV cache with {} blocks\n", .{config.num_blocks});
...
fn benchDecodeBatch(allocator: std.mem.Allocator, batch: usize, seq_len: usize, iters: usize, gpu_only: bool) !struct { cpu_ms: f64, cpu_toks: f64, gpu_ms: f64, gpu_toks: f64 } {
    const config = benchConfig(1024, batch);
    debug.dbg.printLevel(.info, "[bench_pa] decode: creating KV cache with {} blocks\n", .{config.num_blocks});
    var kv = try pa.PagedKVCache.init(allocator, config);
    defer kv.deinit();
    debug.dbg.printLevel(.info, "[bench_pa] decode: KV cache created\n", .{});
    ...
    defer if (has_cuda) cudaz.cuStreamDestroy(gpu_stream);
    if (cudaz.isCudaAvailable()) {
        debug.dbg.printLevel(.info, "[bench_pa] decode: CUDA available, ensuring context\n", .{});
        try cudaz.ensureContext();
        debug.dbg.printLevel(.info, "[bench_pa] decode: context ensured, creating stream\n", .{});
        gpu_stream = try cudaz.cuStreamCreate(0);
        debug.dbg.printLevel(.info, "[bench_pa] decode: stream created: {}\n", .{gpu_stream});
        has_cuda = true;
        debug.dbg.printLevel(.info, "[bench_pa] decode: initializing PagedAttentionGpu\n", .{});
        engine = try pa.PagedAttentionGpu.init(allocator, config, gpu_stream);
        debug.dbg.printLevel(.info, "[bench_pa] decode: PagedAttentionGpu initialized successfully\n", .{});
    }
    ...
    if (!gpu_only) {
        const t = @import("time").Timer.start();
        debug.dbg.printLevel(.info, "[bench_pa] decode: running CPU benchmark loop ({} iters)\n", .{iters});
        for (0..iters) |_| try attn.decodeBatch(queries, outs, bts[0..batch], kv.block_alloc);
        const cpu_ns = t.read();
        debug.dbg.printLevel(.info, "[bench_pa] decode: CPU loop done\n", .{});
        cpu_ms = @as(f64, @floatFromInt(@divTrunc(cpu_ns, std.time.ns_per_ms))) / @as(f64, @floatFromInt(iters));
        cpu_toks = @as(f64, @floatFromInt(batch)) / (cpu_ms / 1000.0);
    }

    if (engine) |*e| {
        debug.dbg.printLevel(.info, "[bench_pa] decode: running GPU benchmark loop ({} iters)\n", .{iters});
        var tg = @import("time").Timer.start();
        for (0..iters) |it| {
            debug.dbg.printLevel(.detail, "[bench_pa] decode: GPU iter {}\n", .{it});
            for (0..batch) |b| {
                debug.dbg.printLevel(.detail, "[bench_pa] decode: GPU iter {} batch {}\n", .{ it, b });
                try e.decode(
        ...
        debug.dbg.printLevel(.info, "[bench_pa] decode: GPU loop done\n", .{});
        const gpu_ns = tg.read();
        gpu_ms = @as(f64, @floatFromInt(@divTrunc(gpu_ns, std.time.ns_per_ms))) / @as(f64, @floatFromInt(iters));
...
pub fn main(init: std.process.Init) !void {
    ...
    debug.dbg.printLevel(.info, "[bench_pa] main: starting benchmark\n", .{});
...
fn benchCpuOffload(allocator: std.mem.Allocator, stdout: anytype) !void {
    ...
    var ba = try pa.BlockAllocator.init(allocator, config);
    defer ba.deinit();
