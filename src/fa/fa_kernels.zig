...
pub const CudaKernel = struct {
    ...
    pub fn unload(self: *CudaKernel) void {
        cudaz.cuModuleUnload(self.module);
    }
};

pub const FlashAttentionBuffers = struct {
    d_q: cudaz.CUdeviceptr,
    d_k: cudaz.CUdeviceptr,
    d_v: cudaz.CUdeviceptr,
    d_o: cudaz.CUdeviceptr,
    bytes: usize,
    ...
    pub fn free(self: *FlashAttentionBuffers) void {
        cudaz.cuMemFree(self.d_q);
        cudaz.cuMemFree(self.d_k);
        cudaz.cuMemFree(self.d_v);
        cudaz.cuMemFree(self.d_o);
    }
};

pub fn computeSharedMemSize(config: FlashAttentionConfig) usize {
    const bq = config.bq;
    const bkv = config.bkv;
    const d = config.d;
...
pub fn launchFlashAttentionV1(kernel: cudaz.CUfunction, config: FlashAttentionConfig, buffers: FlashAttentionBuffers, stream: cudaz.CUstream) !void {
    ...
    var args = .{
        ...
        @as(c_int, @intCast(config.N)),  @as(c_int, @intCast(config.num_heads)), config.scale(),                 @as(c_int, @intFromBool(config.causal)),
        @as(c_int, @intCast(config.bq)), @as(c_int, @intCast(config.bkv)),       @as(c_int, @intCast(config.d)), stream,
    };
    try cudaz.cuLaunchKernel(kernel, launch_config.grid_x, launch_config.grid_y, launch_config.grid_z, launch_config.block_x, launch_config.block_y, launch_config.block_z, @intCast(smem_size), stream, @ptrCast(&args), null);
}

pub fn launchFlashAttentionV2(kernel: cudaz.CUfunction, config: FlashAttentionConfig, buffers: FlashAttentionBuffers, stream: cudaz.CUstream) !void {
    const launch_config = computeLaunchConfig(config);
    const smem_size = computeSharedMemSize(config);
    var args = .{
        &buffers.d_q,                    &buffers.d_k,                     &buffers.d_v,                           &buffers.d_o,
        @as(c_int, @intCast(config.N)),  @as(c_int, @intCast(config.d)),   config.scale(),                         @as(c_int, @intFromBool(config.causal)),
        @as(c_int, @intCast(config.bq)), @as(c_int, @intCast(config.bkv)), @as(c_int, @intCast(config.num_heads)), stream,
    };
    try cudaz.cuLaunchKernel(kernel, launch_config.grid_x, launch_config.grid_y, launch_config.grid_z, launch_config.block_x, launch_config.block_y, launch_config.block_z, @intCast(smem_size), stream, @ptrCast(&args), null);
}
