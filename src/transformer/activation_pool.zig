...
pub const ActivationPool = struct {
    ...
    pub fn alloc(self: *Self, numel: usize) ![]f32 {
        ...
        if (best_idx) |idx| {
            ...
            self.hits += 1;
            // Contrato: devolver EXACTAMENTE `numel` elementos. El buffer
            // interno puede ser mayor (best-fit reusa el libre más ajustado),
            // pero una slice más larga rompe consumers que comparan
            // `.data.len` (for multi-objeto) — regresión hybrid_attn.zig:1050
            // (decode N=1 tras prefill N≥2 heredaba la longitud del prefill).
            return self.entries.items[idx].buffer[0..numel];
        }
    ...
    pub fn reportMetrics(self: *Self) void {
        ...
        const ratio = if (self.hits + self.misses > 0)
            ...
            0.0;
        debug.dbg.printLevel(.detail, "[activation_pool] entries={d} used={d}MB hits={d} misses={d} hit_rate={d:.1}%\n", .{ self.entries.items.len, self.used_bytes / (1024 * 1024), self.hits, self.misses, ratio });
    }
};
...
test "ActivationPool: reuse returns exact requested length" {
    // Regresión hybrid_attn.zig:1050 — decode N=1 tras prefill N=2 pedía 512
    // y recibía el buffer de 1024 del prefill ⇒ panic `non-equal lengths`.
    var pool = ActivationPool.init(testing.allocator, 1024 * 1024);
    defer pool.deinit();

    const prefill_buf = try pool.alloc(1024);
    try testing.expectEqual(prefill_buf.len, 1024);
    pool.release(prefill_buf);

    const decode_buf = try pool.alloc(512);
    try testing.expectEqual(decode_buf.len, 512); // NO 1024
    pool.release(decode_buf);
}
