...
/// Breadcrumb DEBUG_LEVEL>=info: reporta el CUresult numérico de un fallo del
/// driver antes de convertirlo en error.CudaError (que pierde el código).
fn fail(res: anytype, src: std.builtin.SourceLocation) error{CudaError} {
    if (debugz.dbg.at(.info)) {
        const known = @typeInfo(CUresult).@"enum".fields;
        var name: []const u8 = "UNKNOWN";
        inline for (known) |f| {
            if (@intFromEnum(res) == f.value) name = f.name;
        }
        const file = if (std.mem.lastIndexOfScalar(u8, src.file, '/')) |p| src.file[p + 1 ..] else src.file;
        debugz.dbg.print("[gpu_kernels] {s}:{d} fallo driver: {s} ({d})\n", .{ file, src.line, name, @intFromEnum(res) });
    }
...
pub fn cuPointerGetAttribute(data: *anyopaque, attribute: CUpointer_attribute, ptr: CUdeviceptr) !void {
    ...
    if (res != .SUCCESS) return fail(res, @src());
}
...
pub fn cuDeviceInfo(allocator: std.mem.Allocator, dev: CUdevice) !GpuInfo {
    ...
    const res_name = cudalib.cuDeviceGetName(&name_buf, @intCast(name_buf.len), dev);
    if (res_name != .SUCCESS) return fail(res_name, @src());
    ...
    var major: c_int = 0;
    var minor: c_int = 0;
    const res_cc = cudalib.cuDeviceComputeCapability(&major, &minor, dev);
    if (res_cc != .SUCCESS) return fail(res_cc, @src());
    var total_mem: usize = 0;
    const res2 = cudalib.cuDeviceTotalMem_v2(&total_mem, dev);
...
pub fn cuCtxDestroy(ctx: CUcontext) void {
    _ = cudalib.cuCtxDestroy_v2(ctx);
}
...
/// UC-1.3 (lane-cuda): carga un cubin/PTX DESDE MEMORIA (imagen = bytes).
/// Es el camino de integración del JIT NVRTC: nvrtc.compileCubin produce
/// bytes y esto produce el mismo CUmodule que cuModuleLoad(fichero).
pub fn cuModuleLoadData(image: []const u8) !CUmodule {
    var module: CUmodule = undefined;
    const res = cudalib.cuModuleLoadData(&module, image.ptr);
    if (res != .SUCCESS) return fail(res, @src());
    return module;
}

pub fn cuModuleUnload(module: CUmodule) void {
    _ = cudalib.cuModuleUnload(module);
}
...
pub fn cuMemFree(dptr: CUdeviceptr) void {
    _ = cudalib.cuMemFree_v2(dptr);
}
...
pub fn cuMemFreeHost(ptr: *anyopaque) void {
    _ = cudalib.cuMemFreeHost(ptr);
}

pub fn cuMemHostRegister(ptr: *anyopaque, bytes: usize, flags: u32) CUresult {
    const res = cudalib.cuMemHostRegister_v2(ptr, bytes, flags);
    return res;
}

pub fn cuMemHostUnregister(ptr: *anyopaque) CUresult {
...
pub fn cuMemAddressFree(ptr: CUdeviceptr, size: usize) void {
    _ = cudalib.cuMemAddressFree(ptr, size);
}
...
pub fn cuMemRelease(handle: CUmemGenericAllocationHandle) void {
    _ = cudalib.cuMemRelease(handle);
}
...
pub fn cuMemUnmap(ptr: CUdeviceptr, size: usize) void {
    _ = cudalib.cuMemUnmap(ptr, size);
}
...
pub fn cuMemGetAllocationGranularity(granularity: *usize, prop: *CUmemAllocationProp, option: u64) !void {
    const res2 = cudalib.cuMemGetAllocationGranularity(granularity, prop, option);
    if (res2 != .SUCCESS) return fail(res2, @src());
}
...
/// 2.2 (lane-f): memset stream-ordered — el síncrono es ILEGAL dentro de
/// graph capture (CUDA_ERROR_OPERATION_NOT_VALID 901 en cada launch del
/// replay). Los caminos capturables (proyecciones fp8 con atomicAdd) DEBEN
/// usar esta variante.
pub fn cuMemsetD8Async(dst: CUdeviceptr, value: u8, count: usize, stream: CUstream) !void {
    const res = cudalib.cuMemsetD8Async(dst, value, count, stream);
    if (res != .SUCCESS) return fail(res, @src());
}

pub fn cuMemcpyDtoD(dst: CUdeviceptr, src: CUdeviceptr, bytes: usize) !void {
    const res2 = cudalib.cuMemcpyDtoD_v2(dst, src, bytes);
    if (res2 != .SUCCESS) return fail(res2, @src());
}
...
pub fn cuMemcpyDtoH(dst: usize, src: CUdeviceptr, bytes: usize) !void {
    const res2 = cudalib.cuMemcpyDtoH_v2(@ptrFromInt(dst), src, bytes);
    if (res2 != .SUCCESS) return fail(res2, @src());
}
...
pub fn cuStreamCreate(flags: c_uint) !CUstream {
    ...
    const res2 = cudalib.cuStreamCreate(&stream, flags);
    if (res2 != .SUCCESS) return fail(res2, @src());
    return stream;
}

pub fn cuStreamDestroy(stream: CUstream) void {
    _ = cudalib.cuStreamDestroy_v2(stream);
}
...
pub fn cuMemcpyDtoDAsync(dst: CUdeviceptr, src: CUdeviceptr, bytes: usize, stream: CUstream) !void {
    const res2 = cudalib.cuMemcpyDtoDAsync_v2(dst, src, bytes, stream);
    if (res2 != .SUCCESS) return fail(res2, @src());
}
...
pub fn cuStreamEndCapture(stream: CUstream) !CUgraph {
    ...
    const res2 = cudalib.cuStreamEndCapture(stream, &graph);
    if (res2 != .SUCCESS) return fail(res2, @src());
    return graph;
}
...
pub fn cuGraphLaunch(exec: CUgraphExec, stream: CUstream) !void {
    const res2 = cudalib.cuGraphLaunch(exec, stream);
    if (res2 != .SUCCESS) return fail(res2, @src());
}

pub fn cuGraphDestroy(graph: CUgraph) void {
    _ = cudalib.cuGraphDestroy(graph);
}

pub fn cuGraphExecDestroy(exec: CUgraphExec) void {
    _ = cudalib.cuGraphExecDestroy(exec);
}
...
pub fn cuGraphNodeGetType(node: CUgraphNode) !CUgraphNodeType {
    var ty: c_int = 0;
    const res2 = cudalib.cuGraphNodeGetType(node, &ty);
    if (res2 != .SUCCESS) return fail(res2, @src());
...
pub fn cuEventCreate(flags: c_uint) !CUevent {
    ...
    const res2 = cudalib.cuEventCreate(&ev, flags);
    if (res2 != .SUCCESS) return fail(res2, @src());
    return ev;
}

pub fn cuEventDestroy(ev: CUevent) void {
    _ = cudalib.cuEventDestroy(ev);
}
...
pub fn cuEventSynchronize(ev: CUevent) !void {
    const res2 = cudalib.cuEventSynchronize(ev);
    if (res2 != .SUCCESS) return fail(res2, @src());
}

/// 4.8 A2 (Gap 6): ordena a `stream` esperar el último record de `ev` —
/// el puente de sync entre el stream de fetch y el de compute (patrón
/// exllamav3 gpu_worker.py:623-680, vllm stream-ordering).
pub fn cuStreamWaitEvent(stream: CUstream, ev: CUevent, flags: c_uint) !void {
    const res = cudalib.cuStreamWaitEvent(stream, ev, flags);
    if (res != .SUCCESS) return fail(res, @src());
}

/// 4.8 A2: estado del event (0 = ya signaled) — polling sin bloquear para
/// el prefetch N+1 (decidir si el fetch de la ronda anterior terminó).
pub fn cuEventQuery(ev: CUevent) !bool {
    const res = cudalib.cuEventQuery(ev);
    if (res == .SUCCESS) return true;
    if (res == .NOT_READY) return false;
    return fail(res, @src());
}
...
pub fn cuLaunchKernel(
    ...
    const res = cudalib.cuLaunchKernel(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes, hStream, kernelParams, extra);
    if (res != .SUCCESS) {
        // cuGetErrorString escribe un PUNTERO a char* — no rellena el buffer
        // in-place. El patrón anterior imprimía el buffer no inicializado.
        var err_ptr: ?[*:0]const u8 = null;
        _ = cudalib.cuGetErrorString(res, @ptrCast(&err_ptr));
        const err_str = if (err_ptr) |p| std.mem.span(p) else "?";
        debugz.dbg.print("[gpu_kernels] cuLaunchKernel failed: {d} ({s})\n", .{ @intFromEnum(res), err_str });
        return error.CudaError;
    }
}
...
const cudalib = struct {
    ...
    extern "c" fn cuModuleLoadData(module: *CUmodule, image: [*]const u8) CUresult; // UC-1.3 lane-cuda (NVRTC JIT)
    ...
    extern "c" fn cuMemHostRegister_v2(ptr: *anyopaque, bytesize: usize, flags: u32) CUresult;
    extern "c" fn cuMemHostUnregister(ptr: *anyopaque) CUresult;
    ...
    extern "c" fn cuMemsetD8Async(dst: CUdeviceptr, value: u8, count: usize, stream: CUstream) CUresult; // 2.2 lane-f
    ...
    extern "c" fn cuStreamWaitEvent(stream: CUstream, event: CUevent, flags: c_uint) CUresult; // 4.8 A2
    extern "c" fn cuEventQuery(event: CUevent) CUresult; // 4.8 A2
    ...
    extern "c" fn cuMemGetInfo_v2(free_bytes: *usize, total_bytes: *usize) CUresult;
...
pub fn ensureContext() !void {
    ...
    const res = cudalib.cuDevicePrimaryCtxRetain(&ctx, device);

    if (res != .SUCCESS) return fail(res, @src());
    const res2 = cudalib.cuCtxSetCurrent(ctx);
...
pub fn ensureCurrent() !void {
    ...
    const res = cudalib.cuCtxGetCurrent(&cur);

    if (res != .SUCCESS) return fail(res, @src());
    if (cur == g_cuda_ctx.?) return;
    const res2 = cudalib.cuCtxSetCurrent(g_cuda_ctx.?);
...
pub fn getDeviceName(dev: CUdevice, allocator: std.mem.Allocator) ![]u8 {
    var name_buf: [256]u8 = undefined;
    const res_name = cudalib.cuDeviceGetName(&name_buf, name_buf.len, dev);
    if (res_name != .SUCCESS) return fail(res_name, @src());
...
/// T1 VRAM-spec (lane-a): VRAM libre/disponible del contexto actual.
pub fn cuMemGetInfo(free_bytes: *usize, total_bytes: *usize) !void {
    const res = cudalib.cuMemGetInfo_v2(free_bytes, total_bytes);
    if (res != .SUCCESS) return fail(res, @src());
}
