...
pub const cudaDeviceptr = *anyopaque;
...
extern "c" fn cudaMemcpyAsync(dst: ?*anyopaque, src: ?*anyopaque, count: usize, kind: cudaMemcpyKind, stream: cudaStream_t) cudaError_t;

pub const CudaRuntimeError = error{CudaError};

pub fn malloc(bytes: usize) CudaRuntimeError!?*anyopaque {
    var p: ?*anyopaque = null;
    try check(cudaMalloc(&p, bytes));
    return p;
}
...
pub fn memcpyAsync(
    ...
    kind: cudaMemcpyKind,
    stream: cudaStream_t,
) CudaRuntimeError!void {
    try check(cudaMemcpyAsync(dst, src, count, kind, stream));
}
...
pub fn memcpy(
    ...
    try check(cudaMemcpy(dst, src, count, kind));
}
