...
pub const PagedAttention = struct {
    ...
    pub fn decode(
        ...
        return self.decodeLimit(query, out, block_table, block_alloc, block_table.num_tokens, 0);
    }
    ...
    /// `kv_start_pos` (SWA): skip tokens before this position (0 = full context).
    fn decodeLimit(
        self: *const Self,
        query: []const f32,
        out: []f32,
        ...
        for (0..num_blocks) |block_idx| {
            ...
            for (0..tokens_in_block) |t| {
                ...
                if (token_pos < kv_start_pos) continue; // SWA: skip tokens before window
    ...
    pub fn prefill(
        ...
        const q_stride = self.config.num_q_heads * self.config.head_dim;
        for (0..seq_len) |i| {
            // Causal: el token i solo atiende a los tokens [kv_start..i].
            // SWA: kv_start = max(0, i+1-W) where W is the window size.
            const kv_start: usize = if (swa_window) |w| blk: {
                break :blk if (i + 1 > w) i + 1 - w else 0;
            } else 0;
