...
pub const PagedKVCache = struct {
    ...
    pub fn init(gpa: std.mem.Allocator, config: PagedConfig) !Self {
        ...
        block_alloc.* = try BlockAllocator.init(gpa, config);
        var self = Self{
            .allocator = gpa,
            .config = config,
    ...
    /// especulativos, lane-c C3). No-op si new_len >= len actual.
    /// Tras truncar, appendDecode/allocatePrefill reutilizan los bloques
    /// liberados y el siguiente prepareWrite hace COW si el último bloque
    /// conservado está compartido.
    pub fn truncate(self: *Self, seq_id: u64, new_len: usize) !void {
        const bt = self.sequences.getPtr(seq_id) orelse return error.SequenceNotFound;
        try bt.truncate(self.block_alloc, new_len);
    }
