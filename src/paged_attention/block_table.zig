...
pub const BlockTable = struct {
    ...
    pub fn appendToken(self: *Self, block_alloc: *BlockAllocator) !void {
        ...
        if (offset == 0) {
            const phys_id = try block_alloc.alloc() orelse return error.OutOfMemory;
    ...
    /// Trunca la secuencia a `new_len` tokens (rollback de drafts
    /// especulativos, lane-c C3). Libera los bloques que quedan totalmente
    /// fuera y ajusta el contador del último bloque conservado.
    ///
    /// Bloques compartidos (fork/prefix): release() sólo decrementa el
    /// refcount — el propietario restante conserva sus datos intactos.
    /// Los hashes de prefix-cache de bloques liberados quedan obsoletos en
    /// la caché (misma semántica que removeSequence).
    /// Trunca la secuencia a `new_len` tokens (rollback de drafts
    /// especulativos, lane-c C3). Libera los bloques que quedan totalmente
    /// fuera y ajusta el contador del último bloque conservado.
    ///
    /// Si el corte cae DENTRO de un bloque compartido (fork/prefix), se hace
    /// copy-on-write: sin esto, el siguiente appendToken escribiría en la
    /// copia del propietario restante (appendToken no hace COW).
    /// Los hashes de prefix-cache de bloques liberados quedan obsoletos en
    /// la caché (misma semántica que removeSequence).
    pub fn truncate(self: *Self, block_alloc: *BlockAllocator, new_len: usize) !void {
        if (new_len >= self.num_tokens) return;

        const keep_blocks = (new_len + self.block_size - 1) / self.block_size;
        while (self.table.items.len > keep_blocks) {
            const phys_id = self.table.pop().?;
            block_alloc.release(phys_id);
        }

        self.num_tokens = new_len;
        if (keep_blocks > 0) {
            const in_block = new_len - (keep_blocks - 1) * self.block_size;
            var phys_id = self.table.items[keep_blocks - 1];
            if (in_block < self.block_size and block_alloc.blocks[phys_id].isShared()) {
                phys_id = try block_alloc.copyOnWrite(phys_id);
                self.table.items[keep_blocks - 1] = phys_id;
            }
            block_alloc.blocks[phys_id].num_tokens = @intCast(in_block);
        }
    }
