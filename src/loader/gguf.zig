...
pub const GgufError = error{
    ...
    TensorNotFound,
    OutOfMemory,
    /// GGUF truncado: el [offset, offset+bytes) de un tensor excede el
    /// archivo (descarga incompleta). Fail-fast del parse con el culpable.
    TruncatedFile,
};
...
pub const GgmlType = enum(u32) {
    ...
    tq1_0 = 34,
    tq2_0 = 35,
    /// BitNet b1.58 (lane-kvc): ternario {-1,0,+1} 2 bits/valor, 4 valores
    /// por byte empaquetados por grupos de 128 (QK_I2_S=128, 32 bytes/grupo),
    /// con UNA escala f32 por tensor al final del bloque de datos + pad 32B.
    /// Layout verificado contra ggml-model-i2_s.gguf (210 tensores, gaps exactos).
    i2_s = 36,
    mxfp4 = 39,
    q1_0 = 41,
    q2_0 = 42,
...
pub const TensorInfo = struct {
    ...
    pub fn dataBytes(self: *const TensorInfo) usize {
        if (self.dtype == .i2_s) {
            // BitNet i2_s: 2 bits/valor + 1 escala f32 por tensor + pad 32B.
            const raw = (self.numel() * 2 + 7) / 8;
            return ((raw + 4 + 31) / 32) * 32;
        }
...
pub const GgufFile = struct {
    ...
    /// 4.3': parse borrowed (fromBytesBorrowed) — `data` es un préstamo del
    /// buffer pinned copy-once del caller; deinit NO debe liberarlo (el
    /// dueño es HostBank.fromFileCopyOnce + su deinit ordenado).
    owns_data: bool = true,
    ...
    /// 4.3' copy-once: parse TOMANDO PRESTADO el buffer pinned del caller
    /// (HostBank.fromFileCopyOnce). Cero copia, cero mmap — una sola copia
    /// física del fichero en RAM. El caller debe mantener el buffer vivo
    /// hasta tras el deinit de este struct (orden: capas → GgufFile → bank).
    pub fn fromBytesBorrowed(allocator: std.mem.Allocator, data: []const u8) !Self {
    ...
    fn parse(allocator: std.mem.Allocator, data: []const u8) !Self {
        ...
        // F/C v3: validación de bounds por tensor (fail-fast, un pasada).
        // Antes: un GGUF truncado (descarga incompleta) reventaba con panic
        // "index out of bounds" en el PRIMER tensorData() del flujo — sin
        // pista del culpable. Ahora el parse reporta el primer tensor cuyo
        // [offset, offset+dataBytes) excede data.len. Repro real:
        // Qwen3.5-0.8B-BF16 en /ai (208MB faltantes, 40+ tensores OOB).
        if (tensor_count > 0) {
            var tit = tensors.iterator();
            while (tit.next()) |kv| {
                const ti = kv.value_ptr;
                const start = tensor_data_offset + @as(usize, @intCast(ti.offset));
                const end = start + ti.dataBytes();
                if (end > data.len) {
                    // Error fatal de carga (excepción documentada al std.debug
                    // de producción): sin esta pista el panic OOB posterior
                    // no identifica el tensor culpable ni la causa.
                    debugz.dbg.print("[loader] ERROR: tensor '{s}' excede el archivo: end={d} > data.len={d} (GGUF truncado — re-descargar)\n", .{ kv.key_ptr.*, end, data.len });
                    return GgufError.TruncatedFile;
                }
            }
        }
    ...
    /// DFlash: target layers to extract features from (array of layer indices)
    pub fn dflashTargetLayers(self: *const Self) ![]const i32 {
        return if (self.getMeta("attention.target_layers")) |v| {
            const arr = v.asArray() orelse return error.InvalidMetadata;
            var result = try std.ArrayList(i32).initCapacity(self.allocator, arr.items.len);
            for (arr.items) |item| {
                const val = item.asInt() orelse return error.InvalidMetadata;
    ...
    /// DFlash block size (default 16)
    pub fn dflashBlockSize(self: *const Self) usize {
        return if (self.getMeta("dflash.block_size")) |v| {
            switch (v) {
                .uint32 => |bs| return @intCast(bs),
                .int32 => |bs| return @intCast(bs),
                .string => |s| return std.fmt.parseInt(usize, s, 10) catch 16,
                else => return 16,
            }
        } else 16;
    }

    /// DFlash mask token ID (from vocab special_mask_id)
    pub fn dflashMaskToken(self: *const Self) ?u32 {
        // This will be resolved from the tokenizer, not GGUF metadata directly
        _ = self;
        return null;
    }

    /// DFlash2 selector top-k (default 10)
    pub fn dflash2SelectorTopK(self: *const Self) usize {
        return if (self.getMeta("dflash2.selector_top_k")) |v| {
            switch (v) {
                .uint32 => |tk| return @intCast(tk),
                .int32 => |tk| return @intCast(tk),
                else => return 10,
            }
        } else 10;
    }

    /// DFlash2 selector rank (default 128)
    pub fn dflash2SelectorRank(self: *const Self) usize {
        return if (self.getMeta("dflash2.selector_rank")) |v| {
            switch (v) {
                .uint32 => |rk| return @intCast(rk),
                .int32 => |rk| return @intCast(rk),
                else => return 128,
            }
        } else 128;
    }

    /// DSpark specific metadata
    pub fn dsparkMarkovRank(self: *const Self) ?usize {
        return if (self.getMeta("dspark.markov_rank")) |v| {
            switch (v) {
                .uint32 => |rk| return @intCast(rk),
                .int32 => |rk| return @intCast(rk),
                else => return null,
    ...
    pub fn dsparkConfRank(self: *const Self) ?usize {
        return if (self.getMeta("dspark.conf_rank")) |v| {
            switch (v) {
                .uint32 => |rk| return @intCast(rk),
                .int32 => |rk| return @intCast(rk),
                else => return null,
...
/// val = d * qs[i]
/// BitNet b1.58 i2_s (lane-kvc): ternario {-1,0,+1} + escala f32 única
/// al final del bloque de datos. Packing del quantizador CPU de BitNet
/// (ggml-bitnet-mad.cpp:81, QK_I2_S=128): el valor j del grupo g
/// (posición lineal p = 128·g + j) vive en el byte g·32 + (j % 32), en
/// los bits [2·(j/32) .. +2).
///
/// La escala está exactamente en bytes[data_bytes_total] (offset = bytes
/// de datos crudos, sin contar padding). El llamador responsable pasa
/// `bytes` cubriendo el tensor entero (vía tensorData) o, para tests
/// parciales, ajusta el tamaño: la escala se lee en offset
/// `(out.len*2+7)/8` (cola de 4B con la scale del sub-tensor).
pub fn dequantI2_S(bytes: []const u8, out: []f32) void {
    if (out.len == 0) return;
    const data_bytes_needed: usize = (out.len * 2 + 7) / 8;
    if (bytes.len < data_bytes_needed + 4) return; // sin espacio para la escala
    const scale: f32 = @bitCast(std.mem.readInt(u32, bytes[data_bytes_needed..][0..4], .little));
    var p: usize = 0;
    while (p < out.len) : (p += 128) {
        const g = p / 128;
        const n = @min(128, out.len - p);
        for (0..n) |jj| {
            const b = i2sByteAt(bytes, g, jj % 32);
            // Layout del cuantizador real (ggml-bitnet-mad.cpp quantize_i2_s +
            // vec_dot_i2_i8_s_1x1): byte = (c0<<6)|(c1<<4)|(c2<<2)|c3 con
            // c0=j%32, c1=32+j%32, c2=64+j%32, c3=96+j%32 ⇒ shift = 6-2*(j/32).
            const two_bits = (b >> @intCast(6 - 2 * (jj / 32))) & 3;
            const v: f32 = switch (two_bits) {
                0 => -1.0,
                1 => 0.0,
                2 => 1.0,
                else => 0.0,
...
/// Byte `gp` del grupo de 128 valores `g` en el layout del conversor HF
/// (convert-hf-to-gguf-bitnet.py quantize_to_i2_s): bloques CONTIGUOS de 32
/// bytes — byte gp del bloque g en `raw[g*32 + gp]`. Packing del byte:
/// (c0<<6)|(c1<<4)|(c2<<2)|c3 con c_k = valor j del sub-rango [32k, 32k+32)
/// ⇒ shift de decodificación 6-2*(j/32).
///
/// NOTA de arqueología (lane-kvc): hay TRES cuantizadores i2_s en el repo
/// BitNet — (a) C quantize_i2_s / vec_dot del runtime mad (interleaved por
/// grupos de 32 bloques, requiere kcfg.ini), (b) ms-converter (codes 1/2/3
/// = w+2, bit-planes intercalados), (c) HF converter (codes 0/1/2, flat,
/// shift 6-2k). El GGUF bitnet2b real fue generado por (c): la prueba
/// estadística de slots 2-bit muestra {00:34.6%, 01:31.4%, 10:34.4%,
/// 11:0%} — code 3 AUSENTE descarta (b); y (a) requiere kcfg.ini con
/// sección m5120_k2560 que no existe en ningún preset.
pub fn i2sByteAt(bytes: []const u8, g: usize, gp: usize) u8 {
    // Layout FLAT (bloques de 32B contiguos): byte gp del bloque g en
    // raw[g*32 + gp], packing del byte (c0<<6)|(c1<<4)|(c2<<2)|c3 con
    // c_k = valor j del sub-rango [32k, 32k+32) — shift de decodificación
    // 6-2*(j/32). Arqueología (lane-kvc, HANDOFFS 2026-09-08): el GGUF
    // bitnet2b NO puede venir del ms-converter (su preprocess_weights
    // requiere kcfg.ini con sección m5120_k2560 — inexistente en todos
    // los presets) ni del runtime mad interleaved; el signature de slots
    // {00:34.6%, 01:31.4%, 10:34.4%, 11:0%} confirma codes {0,1,2}
    // (3 ausente) con p(−1)≈p(+1)≈35%, p(0)≈31% — BitNet absmean donde
    // la mayoría de pesos redondean a ±1.
    const idx = g * 32 + gp;
    return if (idx < bytes.len) bytes[idx] else 0;
}

/// Q1_0: 1-bit sign per weight, QK=128, 18B/block [d f16][qs 16B].
///   val = sign ? d : -d
pub fn dequantQ1_0(bytes: []const u8, out: []f32) void {
    const block = 128;
    ...
    while (i < out.len) : (i += block) {
        ...
        for (0..n) |j| {
            const byte_idx = j / 8;
            const bit_idx = j % 8;
            const bit = (qs[byte_idx] >> @intCast(bit_idx)) & 1;
            out[i + j] = if (bit != 0) d else -d;
        }
    }
}

/// Q2_0: 2-bit per weight, QK=64, 18B/block [d f16][qs 16B].
///   q ∈ {0,1,2,3} → val = (q - 1) * d  => {-1, 0, +1, +2}
pub fn dequantQ2_0(bytes: []const u8, out: []f32) void {
    const block = 64;
    const block_bytes = 18;
    var i: usize = 0;
    while (i < out.len) : (i += block) {
...
/// Escritura acotada para colas parciales: los bloques SB escriben posiciones
/// fijas [base, base+blockSize); en la última iteración parcial solo son
/// válidas las primeras nv (observación lane-F @aaccc3a sobre q6_K).
inline fn putTail(out: []f32, base: usize, nv: usize, off: usize, v: f32) void {
    if (off < nv) out[base + off] = v;
}
...
pub fn dequantQ4_K(bytes: []const u8, out: []f32) void {
    ...
    while (i < out.len) : (i += qk) {
        ...
        while (j < qk) : (j += 64) {
            ...
            const q = qs[(j / 64) * 32 ..];
            for (0..32) |l| {
                putTail(out, i, nv, j + l, d1 * @as(f32, @floatFromInt(q[l] & 0xF)) - m1);
                putTail(out, i, nv, j + 32 + l, d2 * @as(f32, @floatFromInt(q[l] >> 4)) - m2);
            }
            is += 2;
        }
        nb += 1;
    ...
    while (i < out.len) : (i += qk) {
        ...
        while (n < qk) : (n += 128) {
            ...
            for (0..32) |l| {
                ...
                putTail(out, i, nv, n + l, d * @as(f32, @floatFromInt(@as(i8, @bitCast(sc2[is + 0])))) * q1);
                putTail(out, i, nv, n + l + 32, d * @as(f32, @floatFromInt(@as(i8, @bitCast(sc2[is + 2])))) * q2);
                putTail(out, i, nv, n + l + 64, d * @as(f32, @floatFromInt(@as(i8, @bitCast(sc2[is + 4])))) * q3);
                putTail(out, i, nv, n + l + 96, d * @as(f32, @floatFromInt(@as(i8, @bitCast(sc2[is + 6])))) * q4);
            }
        }
        nb += 1;
    }
        ...
        while (n < qk) : (n += 128) {
            ...
            for (0..4) |j| {
                ...
                for (0..16) |l| {
                    ...
                    putTail(out, i, nv, n + j * 32 + l, dl * @as(f32, @floatFromInt(q1)) - ml);
                    putTail(out, i, nv, n + j * 32 + 16 + l, dl2 * @as(f32, @floatFromInt(q2)) - ml2);
                }
                shift += 2;
            }
        }
        ...
        while (n < qk) : (n += 128) {
            ...
            for (0..4) |j| {
                ...
                for (0..16) |l| {
                    ...
                    putTail(out, i, nv, n + j * 32 + l, dl * @as(f32, @floatFromInt(q1 - h1)));
                    putTail(out, i, nv, n + j * 32 + 16 + l, dl2 * @as(f32, @floatFromInt(q2 - h2)));
                }
                shift += 2;
                m <<= 1;
            }
...
pub fn dequantQ8_K(bytes: []const u8, out: []f32) void {
    ...
    while (i < out.len) : (i += qk) {
        ...
        for (0..qk) |j| {
            putTail(out, i, nv, j, d * @as(f32, @floatFromInt(@as(i8, @bitCast(qs[j])))));
        }
        nb += 1;
    }
}
...
pub fn dequantIq4_xs(bytes: []const u8, out: []f32) void {
    ...
    while (i < out.len) : (i += qk) {
        ...
        for (0..qk / 32) |ib| {
            ...
            const q = qs[ib * 16 ..];
            for (0..16) |j| {
                putTail(out, i, nv, ib * 32 + j, dl * @as(f32, @floatFromInt(kvalues_iq4nl[q[j] & 0xF])));
                putTail(out, i, nv, ib * 32 + j + 16, dl * @as(f32, @floatFromInt(kvalues_iq4nl[q[j] >> 4])));
            }
        }
        nb += 1;
    }
        ...
        for (0..qk / 64) |it| {
            ...
            for (0..4) |l| {
                ...
                for (0..4) |j| {
                    ...
                    putTail(out, i, nv, it * 64 + l * 8 + j, db1 * gridByte(e1, j) * s1);
                    putTail(out, i, nv, it * 64 + l * 8 + j + 4, db1 * gridByte(e2, j) * s2);
                }
            }
            ...
            for (0..4) |l| {
                ...
                for (0..4) |j| {
                    const s1: f32 = if (sm & kmask_iq2xs[j] != 0) -1 else 1;
                    const s2: f32 = if (sm & kmask_iq2xs[j + 4] != 0) -1 else 1;
                    putTail(out, i, nv, it * 64 + 32 + l * 8 + j, db2 * gridByte(e1, j) * s1);
                    putTail(out, i, nv, it * 64 + 32 + l * 8 + j + 4, db2 * gridByte(e2, j) * s2);
                }
            }
        }
        nb += 1;
...
pub fn dequantIq4_nl(bytes: []const u8, out: []f32) void {
    ...
    while (i < out.len) : (i += qk) {
        ...
        for (0..qk / 2) |j| {
            putTail(out, i, nv, j, d * @as(f32, @floatFromInt(kvalues_iq4nl[qs[j] & 0xF])));
            putTail(out, i, nv, j + qk / 2, d * @as(f32, @floatFromInt(kvalues_iq4nl[qs[j] >> 4])));
        }
        nb += 1;
    }
}
        ...
        for (0..qk / 32) |ib| {
            ...
            for (0..4) |l| {
                ...
                for (0..8) |j| {
                    ...
                    putTail(out, i, nv, ib * 32 + l * 8 + j, db * gv * s);
                }
            }
        }
        nb += 1;
        ...
        for (0..qk / 32) |ib| {
            ...
            for (0..4) |l| {
                ...
                for (0..4) |j| {
                    ...
                    putTail(out, i, nv, ib * 32 + l * 8 + j, db * gridByte(g1, j) * s1);
                    putTail(out, i, nv, ib * 32 + l * 8 + j + 4, db * gridByte(g2, j) * s2);
                }
            }
        }
        nb += 1;
        ...
        for (0..qk / 32) |ib| {
            ...
            for (0..4) |l| {
                ...
                for (0..8) |j| {
                    ...
                    putTail(out, i, nv, ib * 32 + l * 8 + j, dl * (gv + dd));
                }
            }
        }
        nb += 1;
    ...
    while (i < out.len) : (i += qk) {
        ...
        for (0..qk / 32) |ib| {
            ...
            for (0..2) |l| {
                ...
                for (0..8) |j| {
                    const gv: f32 = @floatFromInt(@as(i8, @bitCast(@as(u8, @intCast((g >> @intCast(8 * j)) & 0xFF)))));
                    putTail(out, i, nv, ib * 32 + l * 8 + j, dl1 * (gv + dd[l]));
                }
            }
            for (2..4) |l| {
                const g = iq1s_grid[idx[l]];
                for (0..8) |j| {
                    const gv: f32 = @floatFromInt(@as(i8, @bitCast(@as(u8, @intCast((g >> @intCast(8 * j)) & 0xFF)))));
                    putTail(out, i, nv, ib * 32 + l * 8 + j, dl2 * (gv + dd[l]));
                }
            }
        }
        nb += 1;
        ...
        for (0..2) |seg| {
            ...
            for (0..len) |j| {
                for (0..5) |n| {
                    ...
                    putTail(out, i, nv, o, @as(f32, @floatFromInt(xi - 1)) * d);
                    o += 1;
                }
            }
        }
        ...
        for (0..2) |seg| {
            for (0..4) |l| {
                for (0..32) |m| {
                    ...
                    putTail(out, i, nv, seg * 128 + l * 32 + m, @as(f32, @floatFromInt(q - 1)) * d);
                }
            }
        }
        nb += 1;
...
pub fn dequantMxfp4(bytes: []const u8, out: []f32) void {
    ...
    while (i < out.len) : (i += qk) {
        ...
        for (0..qk / 2) |j| {
            putTail(out, i, nv, j, @as(f32, @floatFromInt(kvalues_fp4[qs[j] & 0xF])) * d);
            putTail(out, i, nv, j + qk / 2, @as(f32, @floatFromInt(kvalues_fp4[qs[j] >> 4])) * d);
        }
        nb += 1;
    }
}
    ...
    while (i < out.len) : (i += qk) {
        ...
        while (j < qk) : (j += 64) {
            ...
            for (0..32) |l| {
                ...
                putTail(out, i, nv, j + l, d1 * v1 - m1);
                putTail(out, i, nv, j + 32 + l, d2 * v2 - m2);
            }
            is += 2;
            bit1 <<= 2;
            bit2 <<= 2;
...
pub fn dequantBlock(dtype: GgmlType, bytes: []const u8, out: []f32, elems: usize) void {
    const o = out[0..@min(elems, out.len)];
    switch (dtype) {
        .f32 => @memcpy(o, std.mem.bytesAsSlice(f32, bytes[0 .. o.len * 4])),
        .f16 => for (0..o.len) |i| {
            o[i] = @floatCast(@as(f16, @bitCast(std.mem.readInt(u16, bytes[i * 2 ..][0..2], .little))));
        },
        .bf16 => for (0..o.len) |i| {
            const bits = @as(u32, @as(u32, std.mem.readInt(u16, bytes[i * 2 ..][0..2], .little)) << 16);
            o[i] = @as(f32, @bitCast(bits));
        },
        .f64 => dequantF64(bytes, o),
        .i8 => dequantI8(bytes, o),
        .i16 => dequantI16(bytes, o),
        .i32 => dequantI32(bytes, o),
        .i64 => dequantI64(bytes, o),
        ...
        .tq2_0 => dequantTq2_0(bytes, o),
        .mxfp4 => dequantMxfp4(bytes, o),
        // i2_s: escala GLOBAL por tensor — el bloque es siempre el tensor
        // completo (el caller carga el tensor entero; no existe dequant
        // parcial coherente). dequantI2_S lee la escala del tail del buffer.
...
test "gguf parse header and metadata" {
    ...
    p += 8; // tensor_count
    std.mem.writeInt(u64, buf[p..][0..8], 2, .little);
    p += 8; // metadata_kv_count

    // Metadata 1: general.architecture = "llama" (string)
    writeString(buf, &p, "general.architecture");
    std.mem.writeInt(u32, buf[p..][0..4], @intFromEnum(MetaValueType.string), .little);
    p += 4;
    writeString(buf, &p, "llama");

    // Metadata 2: llama.embedding_length = 4096 (uint64)
    writeString(buf, &p, "llama.embedding_length");
    std.mem.writeInt(u32, buf[p..][0..4], @intFromEnum(MetaValueType.uint64), .little);
    p += 4;
    std.mem.writeInt(u64, buf[p..][0..8], 4096, .little);
    p += 8;
    ...
    p += 8;
    std.mem.writeInt(u32, buf[p..][0..4], @intFromEnum(GgmlType.f16), .little);
    p += 4;
    ...
    std.mem.writeInt(u64, buf[p..][0..8], 4096, .little);
    p += 8;
    std.mem.writeInt(u32, buf[p..][0..4], @intFromEnum(GgmlType.q8_0), .little);
    p += 4;
    std.mem.writeInt(u64, buf[p..][0..8], 4096 * 32000 * 2, .little);
    p += 8;
...
test "gguf metadata array of strings" {
    ...
    std.mem.writeInt(u32, buf[p..][0..4], @intFromEnum(MetaValueType.array), .little);
    p += 4;
    std.mem.writeInt(u32, buf[p..][0..4], @intFromEnum(MetaValueType.string), .little);
    p += 4;
