...
pub const QuantWeight = struct {
    ...
    /// BitNet i2_s (lane-kvc P3): la escala es GLOBAL al tensor (vive en el
    /// tail del buffer, tras los datos crudos), por lo que NO existe dequant
    /// por-bloques coherente — `dequantBlock` con un slice de 32 bytes no
    /// puede verla. Los métodos dequant special-casean este dtype: dequantizan
    /// de una pasada con `gguf.dequantI2_S` sobre el buffer completo (que lee
    /// la escala del tail). `out` debe tener numel() elementos.
    fn isI2S(self: *const Self) bool {
        return self.info.dtype == .i2_s;
    }

    /// Dequant i2_s completo a f32 en `out` (numel() elementos), sin loop
    /// por-bloques. Único path correcto para este dtype.
    fn dequantI2SFull(self: *const Self, out: []f32) void {
        gguf.dequantI2_S(self.bytes, out);
    }

    /// Escala f32 global de un tensor i2_s (tail del buffer, tras los
    /// (numel*2+7)/8 bytes de datos crudos).
    fn i2sScale(self: *const Self) f32 {
        const total: usize = @intCast(self.info.numel());
        const data_bytes = (total * 2 + 7) / 8;
        if (self.bytes.len < data_bytes + 4) return 1.0;
        return @bitCast(std.mem.readInt(u32, self.bytes[data_bytes..][0..4], .little));
    }
    ...
    pub fn dequantToF16(self: *const Self, out: []f16) void {
        ...
        if (self.isI2S()) { // lane-kvc P3: escala global — una sola pasada
            const total: usize = @intCast(info.numel());
            const scale = self.i2sScale();
            var dst: usize = 0;
            // dequantI2_S requiere el buffer COMPLETO (la escala es global
            // al tensor), así que decodificamos inline por grupos de 128
            // contra el buffer entero (el layout del quantizador CPU:
            // valor j del grupo g en byte g*32 + j%32, bits 2*(j/32)).
            while (dst < total) {
                const g = dst / 128;
                const n = @min(128, total - dst);
                for (0..n) |jj| {
                    const b = gguf.i2sByteAt(self.bytes, g, jj % 32);
        ...
        const dt = info.dtype;
        std.debug.assert(@intFromEnum(dt) <= 42);
        const bs = dt.blockSize();
    ...
    pub fn dequantToF16Transposed(self: *const Self, out: []f16) void {
        ...
        if (self.isI2S()) { // lane-kvc P3: escala global, transposición inline
            const total: usize = @intCast(info.numel());
            const scale = self.i2sScale();
    ...
    pub fn dequantToF32(self: *const Self, out: []f32) void {
        const info = self.info;
        if (self.isI2S()) { // lane-kvc P3: escala global — una sola pasada
            return self.dequantI2SFull(out);
        }
        const dt = info.dtype;
        std.debug.assert(@intFromEnum(dt) <= 42);
        const bs = dt.blockSize();
    ...
    pub fn get_subtensor(
        ...
        // lane-kvc P3: i2_s con escala global — decodificar inline por grupos
        // de 128 (el layout del quantizador CPU: valor j del grupo g en byte
        // g*32 + j%32, bits 2*(j/32)). Sin loop dequantBlock (no puede ver
        // la escala).
        if (self.isI2S()) {
            const scale = self.i2sScale();
            const valAt = struct {
                fn f(bytes: []const u8, sc: f32, idx: usize) f32 {
                    const g = idx / 128;
                    const jj = idx % 128;
                    const b = gguf.i2sByteAt(bytes, g, jj % 32);
                    const two_bits = (b >> @intCast(6 - 2 * (jj / 32))) & 3;
                    const v: f32 = switch (two_bits) {
                        0 => -1.0,
                        1 => 0.0,
            ...
            if (info.n_dims == 2) {
                ...
                for (0..rows) |orr| {
                    const c = row_start + orr; // dim1 = output rows
                    for (0..cols) |occ| {
                        const r = col_start + occ; // dim0 = input cols
                        out[orr * cols + occ] = @floatCast(valAt(self.bytes, scale, c * d0 + r));
                    }
                }
            ...
            errdefer allocator.free(out);
            for (0..out_len) |i| out[i] = @floatCast(valAt(self.bytes, scale, col_start + i));
            return Tensor(f16){
                .data = out,
