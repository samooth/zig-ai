...
// New legacy and custom format launchers
...
pub const GgufDequantEngine = struct {
    ...
    pub fn dequant(self: *const Self, dtype: gguf.GgmlType, bytes: []const u8, out: []f32) GgufDequantError!void {
        ...
        // Breadcrumb gated DEBUG_LEVEL>=detail: qué dtype se dequantiza.
        debug.dbg.printLevel(.detail, "[dequant] dtype={s}, bytes.len={d}, out.len={d}\n", .{ @tagName(dtype), bytes.len, out.len });
